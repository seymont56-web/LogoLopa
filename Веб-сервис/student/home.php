...
function game_preview_path(string $gamePath): string
{
  ...
  return $gameDir . '/img/preview.webp';
}
?>
<!doctype html>
<html lang="ru">
...
========================= */
    ...
    #closeGame:hover {
      box-shadow: 0 0 18px rgba(255, 159, 10, .42);
    }
    ...
    .modal.is-loading .modal__body iframe,
    .modal.is-ready .modal__body iframe {
      opacity: 0;
      pointer-events: none;
    }

    .modal.is-playing .modal__body iframe {
      opacity: 1;
      pointer-events: auto;
    }
...
========================= */
    ...
    .game-loader {
      ...
      background:
        radial-gradient(circle at 50% 20%, rgba(255, 159, 10, .18), transparent 34%),
        linear-gradient(135deg, #29209D 0%, #1b1566 100%);
    ...
    .game-loader__card {
      ...
      padding: clamp(26px, 4vw, 46px) clamp(22px, 4vw, 44px);
      border-radius: 30px;
      ...
      background: rgba(238, 238, 255, .96);
      box-shadow:
        0 22px 50px rgba(0, 0, 0, .32),
        inset 0 1px 14px rgba(255, 255, 255, .55);
    }
    ...
    .game-loader__spinner {
      ...
      border-radius: 50%;
      border: 8px solid rgba(41, 32, 157, .12);
      border-top-color: #ff9f0a;

      animation: gameLoaderSpin .9s linear infinite;
    }
    ...
    .game-loader__progress {
      ...
      background: rgba(41, 32, 157, .12);
      box-shadow: inset 0 2px 5px rgba(25, 21, 85, .12);
    }

    .game-loader__progress-bar {
      width: 0%;
      height: 100%;

      border-radius: 999px;

      background: linear-gradient(90deg, #ff9f0a, #ffc15a);
      box-shadow: 0 0 16px rgba(255, 159, 10, .42);
    ...
    .game-loader__play {
      ...
      box-shadow: 0 12px 24px rgba(255, 159, 10, .38);
      transition: transform .2s ease, box-shadow .2s ease;
    }

    .game-loader__play:hover {
      transform: scale(1.04);
      box-shadow: 0 0 22px rgba(255, 159, 10, .55);
    }

    .game-loader__play.is-visible {
      display: flex;
    }

    .game-loader.is-ready .game-loader__spinner {
      display: none;
    }

    .game-loader.is-ready .game-loader__play {
      display: flex;
    }

    .game-loader.is-error .game-loader__spinner {
      display: none;
    }

    .game-loader.is-error .game-loader__progress-bar {
      background: #ff4f4f;
      box-shadow: 0 0 16px rgba(255, 79, 79, .35);
    }

    .game-loader.is-error .game-loader__title {
      color: #d93636;
    }
...
<body class="student-home-page">
  ...
  <header class="student-header">
    ...
    <a class="student-logo" href="../index.php" aria-label="Главная">
  ...
  <div id="gameModal" class="modal hidden" aria-hidden="true">
    <div class="modal__window">
      ...
      <div class="modal__body">
        <div id="gameLoader" class="game-loader hidden">
          <div class="game-loader__card">
            <div class="game-loader__spinner"></div>

            <h2 class="game-loader__title">Загрузка игры</h2>

            <p class="game-loader__text">
              Подготавливаем картинки и звук...
            </p>

            <div class="game-loader__progress">
              <div id="gameLoaderBar" class="game-loader__progress-bar"></div>
            </div>

            <div id="gameLoaderPercent" class="game-loader__percent">
              0%
            </div>

            <button id="startLoadedGame" class="game-loader__play" type="button">
              Играть
            </button>
          </div>
        </div>

        <iframe id="gameFrame" src="" loading="eager" sandbox="allow-scripts allow-same-origin" allow="autoplay"
          title="Окно игры">
        </iframe>
      </div>
    </div>
